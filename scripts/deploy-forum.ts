...
async function main() {
  ...
  const Goflow: ContractFactory = new ethers.ContractFactory('Goflow', [], {});
  const goflow: Contract = await Goflow.deploy() as Contract;
  const Forum: ContractFactory = new ethers.ContractFactory('Forum', {}) as any;
  // pass the GOFLOW token address to the Forum contract's constructor!
  const forum: Contract = await Forum.deploy(goflow.address) as Contract;
   ...
   */
  const Matic = await new ethers.ContractFactory('Matic');
  const matic = await Matic.deploy();
  await matic.deployed();

  console.log('matic deployed to: ', matic.address);
  const AMM = await new ethers.ContractFactory('AMM');
  const amm = await AMM.deploy(matic.address, goflow.address) as Contract;
  await amm.deployed();
...
function makeBig(arg0: number): any | ethers.Overrides {
